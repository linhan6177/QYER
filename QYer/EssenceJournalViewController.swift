...
class EssenceJournalViewController: UITableViewController
{
            ...
            if status == 1
            {
                if let trips = json["data"].arrayValue
                {
                    for i in 0..<trips.count
                    {
                        ...
                        journal.title = trips[i]["subject"].stringValue ?? ""
                        ...
                        journal.likeCount = ((trips[i]["like_count"].stringValue ?? "0") as NSString).integerValue
                        journal.commentCount = ((trips[i]["comment_count"].stringValue ?? "0") as NSString).integerValue
                        journal.viewCount = ((trips[i]["view_count"].stringValue ?? "0") as NSString).integerValue
    ...
    {
        var cell:EssenceJournalCell? = tableView.dequeueReusableCellWithIdentifier("EssenceJournalCell") as? EssenceJournalCell
        if cell == nil
        {
            cell = EssenceJournalCell(style: UITableViewCellStyle.Default, reuseIdentifier: "EssenceJournalCell", cellWidth: self.view.width)
            println("create EssenceJournalCell")
        }
